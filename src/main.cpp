...
void test8() {
  ...
  try {
    bufMgr->disposePage(file6ptr, i);
    PRINT_ERROR(
        "ERROR :: The page is deleted. Exception should have been thrown before execution reaches this point.");
  } catch (InvalidPageException &e) {
  }
...
void test9() {
  std::cout << "[Test 9]" << "\n";
  std::cout
      << "Testing disposePage() and testing readPage() when reading a disposed page."
      << "\n";
  try {
    bufMgr->disposePage(file7ptr, 1);
    bufMgr->readPage(file7ptr, 1, page);
    PRINT_ERROR(
        "ERROR :: The page is invalid. Exception should have been thrown before execution reaches this point.");
  } catch (InvalidPageException &e) {
  }
  std::cout << "Passed" << "\n";
}
...
void test11() {
  ...
  try {
    page->getRecord(rid2);
    PRINT_ERROR(
        "ERROR :: The record has been deleted. Exception should have been thrown before execution reaches this point.");
